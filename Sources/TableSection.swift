...
public extension TableSection
{
    ...
    var firstItem: TableItem? { return item(atIndex: 0) }
    var lastItem: TableItem? { return item(atIndex: numberOfItems - 1) }
    ...
    func index(ofItem theItem: TableItem) -> Int?
    {
        for index in 0..<numberOfItems
        {
            if theItem === item(atIndex: index)
            {
                return index
            }
        }
    ...
    func append(item: TableItem) { append(items: [item]) }
    
    func insert(item: TableItem, at index: Int) { insert(items: [item], at: index) }
...
open class StandardTableSection : StandardTableItem, TableSection
{
    ...
    {
        return ((index >= 0) && (index < numberOfItems)) ? items[index] : nil
    }
